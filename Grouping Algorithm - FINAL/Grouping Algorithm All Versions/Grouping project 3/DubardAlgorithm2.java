...
//Make it so that if the winning array is empty, run the program again but with a smaller size
	...
	public static void findMax() {
		...
		if(validGroups.size() > 1) {
			int random = (int) ((validGroups.size()-1) * Math.random());
				validPairs = validGroups.get(random).getPairs();
				//validGroups.get(random).outputPairs();
		} else {
			validPairs = validGroups.get(0).getPairs();
			//validGroups.get(0).outputPairs();
		}
		
//		System.out.println(input.length);
		if(validPairs.size() < input.length) {
			fillValidPairs(input.length, validPairs);
		}
...
//		System.out.print(correctGrouping + " " + correctGrouping.size());
		
	}
	
	public static void fillValidPairs(int n, ArrayList<String> validPair) {
		...
		for(int i=0; i<n; i++) {
			allLetters.add(numToLetter(i));
		}
		
		for(String string: validPair) {
			usedLetters.add(string.substring(0,1));
			usedLetters.add(string.substring(1));
		}
		...
		for(int i=0; i<availableLetters.size(); i+=2) {
			String pair = availableLetters.get(i) + availableLetters.get(i+1);
			validPair.add(pair);
		}
	}
	...
	public static void createInitial(boolean change) throws FileNotFoundException{
		...
		int[][] scoresHolder = new int[CLASS_SIZE][CLASS_SIZE]; 
		int[][] scoresFinalHolder = new int[CLASS_SIZE][CLASS_SIZE]; 
		
		scores = scoresHolder;
		scoresFinal = scoresFinalHolder;
